...
lm.series <- function(M,design=NULL,ndups=1,spacing=1,weights=NULL) {
  ...
  # Alex Pickering modified May 18 2021.
  ...
  for (i in 1:ngenes) {
    ...
    if(sum(obs) > 0) {
      ...
      if (i == 1) idx <- row(inv) == col(inv)
      stdev.unscaled[i,est] <- sqrt(inv[idx])

      if(df.residual[i] > 0)
        if(is.null(weights))
          sigma[i] <- sqrt(sum(out$residuals^2)/df.residual[i])
      else
        sigma[i] <- sqrt(sum(w*out$residuals^2)/df.residual[i])
    }
  }
  QR <- qr(design)
